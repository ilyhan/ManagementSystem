...
const BoardTask = ({ id, title, preority, assignee, currentNameId }: IBoardTaslProps) => {
  ...
  return (
    <>
      <article onClick={handleOpen} className={`board-task board-task_${preority.toLowerCase()}`}>
        <p className="board-task__id">{currentNameId}</p>

        <UserInfo name={assignee.fullName} />

        <p className="board-task__title">{title}</p>
      </article>

      {open && <UpdateModal taskId={id} open={open} onClose={handleClose} />}
    </>
  );
};
